...
def register_staff():
    print("\n=== Staff Registration ===")
    filename = "staff.json"
    staff_members = load_data(filename)

    while True:
        user_id = input("Enter Staff ID: ")
        if not is_unique_id(staff_members, user_id):
    ...
    while True:
        ...
        if not is_unique_email(staff_members, email):
    ...
    save_data(filename, staff_members)
    print("\nStaff sing_up Successfully!")
    back_to_menu()
...
def login_staff():
    ...
    for staff in staff_members:
        if staff['email'] == email and staff['password'] == password:
            print(f"\nWelcome {staff['name']}! Login Successful.")
            staff_dashboard(staff)
            break
    else:
        print("\nInvalid Email or Password.")
        back_to_menu()
def staff_dashboard(current_staff):
    while True:
        print(f"\n--- Staff Dashboard ---")
        ...
        if choice == '1':
            ...
            if old_pass == current_staff['password']:
            ...
            if confirm.lower() == 'yes':
                staff_members = [staff for staff in staff_members if staff['id'] != current_staff['id']]
                save_data(filename, staff_members)
